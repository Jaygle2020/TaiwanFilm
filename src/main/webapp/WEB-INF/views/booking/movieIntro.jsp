...
.hotList li:first-child figure {
	display: inline-block;
}

.hotList li:first-child section {
	display: inline-block;
	vertical-align: top;
    padding: 150px;
}

.hotList li:not(:first-child) {
	list-style-type: none;
	display: inline-block;
	width: 23%;
	vertical-align: top;
...
<body>
	...
	<ul class="hotList">
		...
		<c:forEach var='movie' items='${movies }'>
			<li>
				<figure>
					<a href="<spring:url value='/movieDetail?id=${movie.movieId}' />"><img
						src="<c:url value='/getPicture/movieBean/${movie.movieId }' />" alt="${movie.movieName }"
						title="${movie.movieName }"></a>
				</figure>
				<section class="infoArea">
					<h2>
						<a class="hotA" href="<spring:url value='/movieDetail?id=${movie.movieId}' />">${movie.movieName}</a>
					</h2>
					<h3 class="ename">${movie.englishName}</h3>
					<div class="rankArea">
						<h4 class="rank">
							NO.<c:out value="${count}" />
						</h4>
					</div>
				</section>
			</li>
